...
| E006 | 2026-07-01 | Plant detail UI stuck forecasting harvest at "Day ~59" regardless of actual plant timeline. | `lib/providers/prediction_providers.dart` | The `forecastProvider` was artificially limited to `forecastDays: 60`, which caused the simulation to cap out early, repeating Day 59 for all later stages (since tomatoes take 90+ days). | Increased `forecastDays` limit to 180 and removed hardcoded 58-day string in `add_plant_sheet.dart` to use dynamic CA prediction. | ✅ Resolved | 15 mins |
| E007 | 2026-07-01 | Build failed: `The argument type 'int' can't be assigned to the parameter type 'double'` | `lib/features/plant_form/presentation/add_plant_sheet.dart` | Attempted to pass `(_light * 4 + 1).round()` (which yields an `int`) into `sunlightScore` of the `Plant` constructor which strictly expects a `double`. | Changed `.round()` to `.roundToDouble()` to maintain the 1-5 scale mapping while satisfying the type checker. | ✅ Resolved | 5 mins |
