...
| F006 | **Plant Dashboard** | Multi-plant tracking interface allowing users to manage several plants simultaneously. Each plant has its own profile with input data, predictions, and health status. List/grid view of all tracked plants. | 🟡 Nice-to-Have | ⏳ Pending Client | Phase 3 |
...
| F008 | **Data Persistence** | Local storage solution to save plant profiles, prediction history, and user preferences across app sessions. No cloud sync — purely on-device storage. Candidates: sqflite, Hive, or drift. | 🟡 Nice-to-Have | ⏳ Pending Client | Phase 2 |
